...
class BaseDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        # 根据index_version替换索引文件名中的{version}占位符
        index_version = getattr(args, 'index_version', 'baseline')
        self.index_file = args.index_file.replace('{version}', index_version)
        self.image_index_file = args.image_index_file.replace('{version}', index_version)
...
class ItemImageDataset(BaseDataset):
    ...
    def __init__(self, args, task="item2image", prompt_sample_num=1, sample_num=-1):
        ...
        # 注意: self.image_index_file 已在 BaseDataset.__init__ 中处理版本替换，不要覆盖
