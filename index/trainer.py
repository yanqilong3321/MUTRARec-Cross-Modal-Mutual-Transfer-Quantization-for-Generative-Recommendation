...
class Trainer(object):
    ...
    def __init__(self, args, model):
        ...
        self.logger.info(f"Model Parameters: Total={total_params/1e6:.2f}M, Trainable={trainable_params/1e6:.2f}M")
    ...
    def fit(self, data):
        ...
        for epoch_idx in range(self.epochs):
            ...
            # Log with detailed timing information
            print(f'Epoch {epoch_idx}: Time {epoch_time:.2f}s, Total Loss: {train_loss:.4f}, Recon Loss: {train_recon_loss:.4f}, RQ Loss: {total_rq_loss:.4f}')
            self.logger.info(f'Epoch {epoch_idx}: Time {epoch_time:.2f}s, Total Loss: {train_loss:.4f}, Recon Loss: {train_recon_loss:.4f}, RQ Loss: {total_rq_loss:.4f}')
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_end_time = time()
                eval_time = valid_end_time - valid_start_time
                ...
                print(f'Eval (Epoch {epoch_idx}): Time {eval_time:.2f}s, MAX: {max_value}, Min: {min_value}, Collision: {collision_rate:.4f}, Best: {self.best_collision_rate:.4f}')
                self.logger.info(f'Eval (Epoch {epoch_idx}): Time {eval_time:.2f}s, Collision: {collision_rate:.4f}, Best: {self.best_collision_rate:.4f}')
                ...
                valid_score_output = (
                    ...
                    + ": %f]"
                ) % (epoch_idx, eval_time, collision_rate)
