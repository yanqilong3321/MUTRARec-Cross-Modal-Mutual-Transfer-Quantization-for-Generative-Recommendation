...
class FusionRQVAE_v4(nn.Module):
    """
    FusionRQVAE V4: 量化前LoRA融合 + Gating门控机制 + 低秩共享码本
    
    核心改进:
    - 引入 Gating Mechanism 替代固定的 fusion_lora_alpha
    - 自动学习每个维度的融合强度
    ...
    def __init__(self,
                 ...
                 # fusion_lora_alpha removed, using gating mechanism
                 layers=None,
                 dropout_prob=0.0,
                 bn=False,
                 loss_type="mse",
        ...
        super(FusionRQVAE_v4, self).__init__()
        ...
        # self.fusion_lora_alpha = fusion_lora_alpha # Removed
        self.layers = layers
        ...
        # 自动决定融合强度：Gate(cat(z, delta)) -> [0, 1]
        self.text_fusion_gate = nn.Sequential(
            nn.Linear(e_dim * 2, e_dim),
            nn.Sigmoid()
        )
        self.vis_fusion_gate = nn.Sequential(
            nn.Linear(e_dim * 2, e_dim),
            nn.Sigmoid()
        )
    ...
    def forward(self, x_text, x_vis, use_sk=True):
        ...
        # 2. Feature-level LoRA Fusion (量化前融合) + Gating
        delta_text = self.text_fusion_lora_A(self.vis_fusion_lora_B(z_vis))
        gate_text = self.text_fusion_gate(torch.cat([z_text, delta_text], dim=-1))
        z_text_fused = z_text + gate_text * delta_text
        
        delta_vis = self.vis_fusion_lora_A(self.text_fusion_lora_B(z_text))
        gate_vis = self.vis_fusion_gate(torch.cat([z_vis, delta_vis], dim=-1))
