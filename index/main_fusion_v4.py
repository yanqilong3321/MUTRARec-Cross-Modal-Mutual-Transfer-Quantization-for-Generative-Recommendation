...
from models.rqvae_v4 import FusionRQVAE_v4
from utils import ensure_dir, set_color

class FusionTrainer_v4(object):
    """FusionRQVAE V4 训练器（量化前融合 + Gating + 低秩共享码本）"""
    ...
    def __init__(self, args, model):
        ...
        self.best_collision_ckpt = "best_collision_model_v4.pth"
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        iter_data = tqdm(train_data, total=len(train_data), ncols=100, 
                        desc=set_color(f"Train {epoch_idx} [V4]", "pink"),
    ...
    def _valid_epoch(self, valid_data):
        ...
        iter_data = tqdm(valid_data, total=len(valid_data), ncols=100,
                        desc=set_color(f"Evaluate [V4]", "pink"),
                        disable=not sys.stdout.isatty())
    ...
    def _save_checkpoint(self, epoch, collision_rate, ckpt_file=None):
        ckpt_path = os.path.join(self.ckpt_dir, ckpt_file) if ckpt_file else \
            os.path.join(self.ckpt_dir, f'epoch_{epoch}_col_{collision_rate:.4f}_model_v4.pth')
        ...
        self.logger.info(set_color("Saved V4 checkpoint", "blue") + f": {ckpt_path}")
    ...
    def fit(self, data_loader):
        ...
        self.logger.info(set_color(f"Fusion: Gating Mechanism", "yellow"))
        self.logger.info(set_color("="*60, "yellow"))
...
def parse_args():
    ...
    parser.add_argument("--ckpt_dir", type=str, default="./log/fusion_v4")
    parser.add_argument("--log_file", type=str, default="train_v4.log",
...
if __name__ == '__main__':
    ...
    logger.info("="*60)
    logger.info("FusionRQVAE V4: Pre-Quant Fusion + Gating + Low-Rank Codebook")
    logger.info("="*60)
    ...
    logger.info(f"Initializing FusionRQVAE_v4: Text={data.text_dim}, Vis={data.vis_dim}")
    logger.info(f"Codebook Rank={args.codebook_lora_rank}, Fusion Method=Gating")
    
    model = FusionRQVAE_v4(
        ...
        # fusion_lora_alpha=args.fusion_lora_alpha, # Removed
        layers=args.layers,
    ...
    trainer = FusionTrainer_v4(args, model)
