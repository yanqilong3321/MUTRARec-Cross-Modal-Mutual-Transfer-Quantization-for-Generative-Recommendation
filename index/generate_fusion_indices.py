...
def parse_args():
    parser = argparse.ArgumentParser(description="Fusion Index Generation")
...
if __name__ == "__main__":
    ...
    # Load Data (Paired)
    # We need to use PairedEmbDatasetAll but effectively for one dataset
    print(f"Loading data for {cmd_args.dataset}...")
    dataset = PairedEmbDatasetAll(train_args) 
    
    # Reconstruct Model
    print("Reconstructing FusionRQVAE...")
    model = FusionRQVAE(
    ...
    for x_text, x_vis in tqdm(data_loader):
        ...
        # We need to manually call the components to get distances, 
        # because model.get_indices only returns indices
        # So we replicate the logic inside FusionRQVAE.get_indices but exposing distances
        
        with torch.no_grad():
            z_text = model.text_encoder(x_text)
            z_vis = model.vis_encoder(x_vis)
            
            delta_text = model.text_lora_A(model.vis_lora_B(z_vis))
            z_text_fused = z_text + model.lora_alpha * delta_text
            
            delta_vis = model.vis_lora_A(model.text_lora_B(z_text))
            z_vis_fused = z_vis + model.lora_alpha * delta_vis
            
            _, _, idx_text, dist_text = model.text_rq(z_text_fused, use_sk=False)
            _, _, idx_vis, dist_vis = model.vis_rq(z_vis_fused, use_sk=False)
    ...
    # Resolve Collisions and Save Text (原版带后缀)
    ...
    save_json(text_indices, os.path.join(cmd_args.output_dir, f"{cmd_args.dataset}.index_lemb_baseline.json"), prefix_text)
    
    # Resolve Collisions and Save Vis (原版带后缀)
    ...
    save_json(vis_indices, os.path.join(cmd_args.output_dir, f"{cmd_args.dataset}.index_vitemb_baseline.json"), prefix_vis)
