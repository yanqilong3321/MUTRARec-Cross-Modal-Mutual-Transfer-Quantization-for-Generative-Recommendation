...
from models.rqvae_v1 import FusionRQVAE_v1
from utils import ensure_dir, set_color

class FusionTrainer_v1(object):
    """FusionRQVAE V1 训练器（低秩共享码本）"""
    ...
    def __init__(self, args, model):
        ...
        self.best_collision_ckpt = "best_collision_model_v1.pth"
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        iter_data = tqdm(train_data, total=len(train_data), ncols=100, 
                        desc=set_color(f"Train {epoch_idx} [V1-LowRank]", "pink"))
    ...
    def _valid_epoch(self, valid_data):
        ...
        iter_data = tqdm(valid_data, total=len(valid_data), ncols=100,
                        desc=set_color(f"Evaluate [V1-LowRank]", "pink"))
        ...
        def get_collision(indices_list):
            ...
            usage_rate = len(indices_set) / num_sample
    ...
    def _save_checkpoint(self, epoch, collision_rate, ckpt_file=None):
        ckpt_path = os.path.join(self.ckpt_dir, ckpt_file) if ckpt_file else \
            os.path.join(self.ckpt_dir, f'epoch_{epoch}_col_{collision_rate:.4f}_model_v1.pth')
        ...
        self.logger.info(set_color("Saved V1 checkpoint", "blue") + f": {ckpt_path}")

    def fit(self, data_loader):
        self.logger.info(set_color("="*60, "yellow"))
        self.logger.info(set_color("Training FusionRQVAE V1 (Low-Rank Shared Codebook)", "yellow"))
        self.logger.info(set_color(f"Init Method: {self.args.codebook_init_method}", "yellow"))
        self.logger.info(set_color(f"LoRA Rank: {self.args.lora_rank}", "yellow"))
        self.logger.info(set_color("="*60, "yellow"))
        ...
        for epoch_idx in range(self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # 定期打印码本统计
                if (epoch_idx + 1) % (self.eval_step * 10) == 0:
                    self.model.print_codebook_stats()
                    
        return self.best_loss, self.best_collision_rate

def parse_args():
    parser = argparse.ArgumentParser(description="Fusion Index V1 - Low-Rank Shared Codebook")
    ...
    parser.add_argument('--lora_rank', type=int, default=8, 
                       help='Low-rank dimension for codebook factorization')
    parser.add_argument('--codebook_init_method', type=str, default='random',
                       choices=['random', 'kmeans_svd'],
                       help='Codebook initialization: random or kmeans_svd')
    
    # 其他参数
    parser.add_argument("--device", type=str, default="cuda:0", help="device")
    parser.add_argument("--ckpt_dir", type=str, default="./log/fusion_v1", help="checkpoint directory")
...
if __name__ == '__main__':
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(args.ckpt_dir, 'train_v1.log')),
    ...
    logger.info("="*60)
    logger.info("FusionRQVAE V1: Low-Rank Shared Codebook")
    logger.info("="*60)
    ...
    # 初始化模型
    logger.info(f"Initializing FusionRQVAE_v1 with Text Dim={data.text_dim}, Vis Dim={data.vis_dim}")
    logger.info(f"Codebook Init Method: {args.codebook_init_method}")
    logger.info(f"LoRA Rank: {args.lora_rank}")
    
    model = FusionRQVAE_v1(
    ...
    # 训练
    trainer = FusionTrainer_v1(args, model)
