...
def parse_args():
    parser = argparse.ArgumentParser(description="Fusion Index Generation V1 (Low-Rank)")
...
if __name__ == "__main__":
    ...
    # Reconstruct Model (V1)
    print("Reconstructing FusionRQVAE_v1 (Low-Rank)...")
    ...
    print("Generating indices (V1: low-rank shared codebook)...")
    text_indices = []
    vis_indices = []
    text_distances = []
    vis_distances = []
    ...
    for x_text, x_vis in tqdm(data_loader):
        ...
        with torch.no_grad():
            ...
            _, _, idx_text, dist_text = model.text_rq(
                z_text, model.codebook_lora_a_list, use_sk=False
            )
            _, _, idx_vis, dist_vis = model.vis_rq(
                z_vis, model.codebook_lora_a_list, use_sk=False
            )
    ...
    # Resolve Collisions and Save (V1版本带后缀)
    ...
    save_json(text_indices, os.path.join(cmd_args.output_dir, f"{cmd_args.dataset}.index_lemb_v1.json"), prefix_text)
    ...
    save_json(vis_indices, os.path.join(cmd_args.output_dir, f"{cmd_args.dataset}.index_vitemb_v1.json"), prefix_vis)
