...
def parse_args():
    parser = argparse.ArgumentParser(description="Fusion Index Generation V4")
...
if __name__ == "__main__":
    ...
    print("Reconstructing FusionRQVAE_v4...")
    model = FusionRQVAE_v4(
        ...
        # fusion_lora_alpha=train_args.fusion_lora_alpha, # Removed in V4
    ...
    print("Generating indices (V4: pre-quant fusion + gating + low-rank codebook)...")
    text_indices = []
    vis_indices = []
    text_distances = []
    vis_distances = []
    ...
    for x_text, x_vis in tqdm(data_loader):
        ...
        with torch.no_grad():
            # 使用模型内置的 get_indices 方法，确保逻辑一致
            # 如果之前没有用 get_indices，现在改用它更安全
            # 或者手动复制 Gating 逻辑
            
            z_text = model.text_encoder(x_text)
            z_vis = model.vis_encoder(x_vis)
            
            # 特征融合 + Gating (复制自 FusionRQVAE_v4.forward)
            delta_text = model.text_fusion_lora_A(model.vis_fusion_lora_B(z_vis))
            gate_text = model.text_fusion_gate(torch.cat([z_text, delta_text], dim=-1))
            z_text_fused = z_text + gate_text * delta_text
            
            delta_vis = model.vis_fusion_lora_A(model.text_fusion_lora_B(z_text))
            gate_vis = model.vis_fusion_gate(torch.cat([z_vis, delta_vis], dim=-1))
            z_vis_fused = z_vis + gate_vis * delta_vis
    ...
    save_json(text_indices, os.path.join(cmd_args.output_dir, f"{cmd_args.dataset}.index_lemb_v4.json"), prefix_text)
    ...
    save_json(vis_indices, os.path.join(cmd_args.output_dir, f"{cmd_args.dataset}.index_vitemb_v4.json"), prefix_vis)
