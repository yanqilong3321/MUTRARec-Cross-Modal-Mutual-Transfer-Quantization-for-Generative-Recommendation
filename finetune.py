...
from transformers.trainer_callback import EarlyStoppingCallback, TrainerCallback  # 导入 TrainerCallback

from utils import *
from collator import Collator

# 定义 NNI 回调，用于报告中间结果给 Assessor
class NNICallback(TrainerCallback):
    def __init__(self, enabled=False):
        """
        Args:
            enabled: 是否启用NNI报告（只有在NNI trial中才启用）
        """
        super().__init__()
    ...
    def on_evaluate(self, args, state, control, metrics, **kwargs):
        """
        在每次评估结束时调用，报告 eval_loss 给 NNI
        只有在 enabled=True 且为主进程时才报告
        """
        if not self.enabled:
            return
        
        # 多GPU训练时，只在主进程（rank 0）报告
        ...
        if eval_loss is not None:
            # NNI Assessor 配置为 maximize，所以我们报告 -loss
            # 这样 loss 越小，分数越高
            print(f"[NNI] Reporting intermediate result: {-eval_loss} (eval_loss: {eval_loss})")
            try:
                nni.report_intermediate_result(-eval_loss)
            except Exception as e:
                print(f"[NNI] Warning: Failed to report intermediate result: {e}")
...
def train(args):
    ...
    # 检查是否在 NNI trial 环境中
    # 通过环境变量 NNI_TRIAL_JOB_ID 判断
    is_nni_trial = os.environ.get('NNI_TRIAL_JOB_ID') is not None
    nni_callback = NNICallback(enabled=is_nni_trial)  # 只在 NNI trial 中启用
    
    if local_rank == 0 and is_nni_trial:
        print(f"[NNI] Running in NNI trial mode, intermediate results will be reported")
    ...
    trainer = transformers.Trainer(
        ...
        callbacks=[early_stop, nni_callback]  # 添加 NNI 回调
    )
