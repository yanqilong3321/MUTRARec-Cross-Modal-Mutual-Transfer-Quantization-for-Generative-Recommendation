...
def main(args):
    ...
    # 替换索引文件名中的{version}占位符
    index_file = args.index_file.replace('{version}', args.index_version)
    image_index_file = args.image_index_file.replace('{version}', args.index_version)

    index_text = os.path.join(args.data_path, args.dataset, f'{args.dataset}{index_file}')
    index_image = os.path.join(args.data_path, args.dataset, f'{args.dataset}{image_index_file}')
...
def parse_args():
    ...
    parser.add_argument("--index_version", type=str, default="baseline", 
                        choices=["baseline", "v1", "v2", "v3", "v4"],
                        help="Index version to use")
    parser.add_argument("--index_file", type=str, default=".index_lemb_{version}.json", help="Input data path.")
    parser.add_argument("--image_index_file", type=str, default=".index_vitemb_{version}.json", help="Input data path.")
